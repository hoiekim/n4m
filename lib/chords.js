const getBase = (notes) => {
  ...
  if (lowKey >= 48) {
    ...
    while (1 < notes.length && i < notes.length) {
      ...
      if (basses[note1] === undefined) basses[note1] = -(note1 / 6);
      if (basses[note2] === undefined) basses[note2] = -(note2 / 6);
      ...
      if (remainder === 5) basses[note2] += 6 / octave;
      if ([3, 4].includes(remainder)) basses[note1] += 5 / octave;
      if ([8, 9].includes(remainder)) basses[note2] += 3 / octave;
